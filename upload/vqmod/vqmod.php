...
abstract class VQMod {
    public static $_vqversion = '2.6.4';						// Current version number JTI UPDATE PHP v.7.3.x
    ...
    //MX
    public static $mx_checkTime = false; // Time to check original files timestamps
    public static $mx_build = false; // Time to check original files timestamps
    ...
    public static function MxChecksum() {
        $vqmods = glob(self::$_cwd . 'vqmod/xml/*.xml');
        $sum = 0;
    ...
    public static function MxPreload() {
        static $done; if ($done) return; // Double preload check

        // From bootup function - preload
        $replacesPath = self::path(self::$pathReplaces);
        $replaces = array();
        if($replacesPath) {
            include_once($replacesPath);
        ...
        self::_loadProtected();
        self::_loadChecked();
        // End From bootup function
    }

    public static function MxRebuild() {

        static $done; if ($done) return; // Double start check

        if (file_exists(self::path(self::$modCache))) unlink(self::path(self::$modCache));
        if (file_exists(self::path(self::$checkedCache))) unlink(self::path(self::$checkedCache));
        self::MxPreload();
        ...
        foreach(self::$_mods as $modObject) {
            foreach($modObject->mods as $path => $mods) $list[]=$path;
        }
        $list=array_unique($list);
        $files=array();
        foreach ($list as $l)
            if ($add = glob($l)) $files = array_merge($files, $add);

        $files=array_unique($files);
        // End build list

        self::$mx_build = true;
        $old_files = glob(self::$_cwd . self::$vqCachePath.'vq2*');
        foreach ($old_files as $file) unlink($file);
        $log_files = glob(self::$_cwd . self::$logFolder.'*.log');
        foreach ($log_files as $file) unlink($file);

        foreach ($files as $current_file) {
            if (!is_file($current_file)) continue;
            $ext=substr($current_file,-3);
            if ( ($ext!="php") and (($ext!="tpl")) ) continue;
            $sourceFile=substr($current_file,strlen(DIR_SYSTEM)-7);
            self::modCheck($sourceFile);
        }

        $mx_file = self::$_cwd . 'vqmod/pro.cache';
        file_put_contents($mx_file, self::MxChecksum());
    }

    public static function MxCheck() { // Checks if there is a need for cache rebuild

        self::$_cachePathFull = self::path(self::$vqCachePath);

        // Lets check for delay
        $mx_file = self::$_cwd . 'vqmod/pro.cache';
        if (!file_exists($mx_file)) {
            self::MxRebuild();
            return;
        }
        $check_every = 3; // SECS - check everything once in this interval
        if (filemtime($mx_file) > time() - $check_every) return;
        self::$mx_checkTime = true;
        if (!count(glob(self::$_cwd . self::$vqCachePath.'vq2*')) || self::MxChecksum() != file_get_contents($mx_file)) self::MxRebuild();
        touch($mx_file);
    }
    //MXE
    ...
    public static function modCheck($sourceFile, $modificationFile = false) {
        ...
        if (!self::$mx_build) {
            ...
            if (substr($sourceFile,0, strlen($root)) == $root) {
                // Fast search
                $cache_file = self::$_cachePathFull . 'vq2-' . str_replace('/', '_', substr($sourceFile, strlen(DIR_SYSTEM) - 7));

                if (!file_exists($cache_file)) return $sourceFile;
                if (!self::$mx_checkTime || filemtime($cache_file)>=filemtime($sourceFile))
                    return $cache_file;
            }
        } else
            //MXE
        ...
        //MXS
        if (defined('DIR_MODIFICATION') and !$modificationFile) {
            $mod = str_replace(substr(DIR_SYSTEM,0,-7), DIR_MODIFICATION, $sourcePath);
            if (file_exists($mod)) $sourcePath = $mod;
        }
        //MXE
        ...
        if (!self::$mx_build) {
            if (file_exists($cacheFile)) {
                if (!self::$mx_checkTime) return $cacheFile;
                if (filemtime($cacheFile)>=filemtime($sourceFile)) return $cacheFile;
                self::MxPreload();
            }
...
// JTI MOD VqMod v.2.6.4 Qphoria PHP 7.3 Fix
							// continue 2;
							break 2;
// END JTI MOD
