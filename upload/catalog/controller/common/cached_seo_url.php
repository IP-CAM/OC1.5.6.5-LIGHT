...
class ControllerCommonSeoUrl extends Controller {
	...
	$this->cache->set('seo_url', $cache_data );
	}
	}
	...
	if (isset($this->cache_data['keywords'][$part])) {$this->sorgu=0;$query = $this->cache_data['keywords'][$part]; 
	$url = explode('=', $query);
	} else {
	$this->sorgu=1;
	...
	if ($url[0] != 'information_id' && $url[0] != 'manufacturer_id' && $url[0] != 'category_id' && $url[0] != 'product_id') {
	...
	if ($query->num_rows && $query->row['keyword']) {
	$url .= '/' . $query->row['keyword'];
	//echo $query->row['keyword']."<br>";
	} 
	}	
	elseif (isset($this->cache_data['queries']['category_id=' . (int)$category])) {
