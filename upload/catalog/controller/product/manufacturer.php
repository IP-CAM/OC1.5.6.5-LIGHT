...
class ControllerProductManufacturer extends Controller {  
	public function index() { 
		...
		foreach ($results as $result) {
			...
			'logo' => $this->model_tool_image->resize($result['image'], 74, 74),
			'name' => $result['name'],
...
// JTI MOD Adding canonical URLs - https://www.opencart.com/index.php?route=marketplace/extension/info&extension_id=16258
...
// JTI MOD Manufacturer Image Size
			$this->data['logo'] = $this->model_tool_image->resize($manufacturer_info['image'],50,50); 
// END JTI MOD

		if ($manufacturer_info) {
			$this->document->setTitle($manufacturer_info['name']);

// JTI MOD JS added to MASTER JS File as permanent Code Part
...
// END JTI MOD
			...
			if (isset($this->request->get['page'])) {
				$url .= '&page=' . $page = (int)$this->request->get['page'];
			}	
			...
			$limits = array_unique(array($this->config->get('config_catalog_limit'), 18, 24, 36));
...
// JTI MOD SEO Pagination: rel next/prev BiteMedia Dev Team
