	...
	$(document).ready(function(){
		$('#continue-button').click(function(){
			...
			if ($('input[name="addressSelected"]').val() == '0') {
				$('#amazon-address-widget').before('<div class="alert alert-danger"><?php echo $error_shipping_address ?></div>');
			} else if($('input[name="shipping_method"]:checked').length == 0) {
				$('#amazon-address-widget').before('<div class="alert alert-danger"><?php echo $error_shipping ?></div>');
			} else {
				$.ajax({
		...
		new CBA.Widgets.AddressWidget({
			...
			onAddressSelect: function(widget) {
				...
				$.get('<?php echo $shipping_quotes ?>', {}, function(data) {
					...
					if (data.error) {
						$('#amazon-address-widget').before('<div class="alert alert-danger">' + data.error + '</div>');
					} else if (data.quotes) {
						var html = '';
						...
						$.each(data.quotes, function(code, shippingMethod){
							...
							$.each(shippingMethod.quote, function(i, quote){
								...
								if (data.selected == quote.code) {
									html += ' <td><input type="radio" name="shipping_method" value="' + quote.code + '" id="' + quote.code + '" checked=""></td>';
								} else {
