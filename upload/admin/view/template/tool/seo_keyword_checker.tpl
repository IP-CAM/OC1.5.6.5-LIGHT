...
input.save {
	...
	height: 26px;
	padding: 10px;
	background: url('view/image/reload.png');
	background-size:100%;
	border: 0px;
}
input.save:hover {
...
input.delete {
	...
	height: 26px;
	padding: 10px;
	background: url('view/image/remove.png');
	background-size:100%;
	border: 0px;
}
input.delete:hover {
...
<div id="content">
	...
	<div class="box">
		<div class="heading">
			<h1><img src="view/image/tool/<?php echo $tool; ?>.png" alt="<?php echo $tool; ?>" width="22" height="22" /> <?php echo $heading_title; ?></h1>
			<div class="buttons">
...
$('#find_duplicates_ctrl').live('click', function(){
	...
	var close_image = '<img src="view/image/close.png" alt="" class="close">';
	$.ajax({
			...
			success: function(data) {
				...
				if (data.error) {
					...
					$.each(data.duplicates, function(i, item) {
						...
						html += '    </td>';
						html += '    <td class="center" style="width:80px;">';
